...
const BlogStoreProvider = ({children}) => {
    ...
    useEffect(() => {
        const postSignup = async(user) => {
            try{
                await axios.post('https://blogs-mern-v9tk.onrender.com/auth/signup', {
                    ...user
                })
    ...
    useEffect(() => {
        const postLogin = async(user) => {
            try{
                const {data} = await axios.post('https://blogs-mern-v9tk.onrender.com/auth/login', {
                    ...user
                })
    ...
    useEffect(() => {
        ...
        const fetchPostList = async() => {
            try{
                const {data} = await axios.get('https://blogs-mern-v9tk.onrender.com/blogs/read', {signal});
    ...
    useEffect(() => {
        const addNewPosts = async(blog) => {
            try{
                const {data} = await axios.post('https://blogs-mern-v9tk.onrender.com/blogs/new', {
    ...
    useEffect(() => {
        const removePosts = async(id) => {
            try{
                await axios.delete(`https://blogs-mern-v9tk.onrender.com/blogs/remove/${id}`);
    ...
    useEffect(() => {
        const updatePosts = async({author, title, body, tags, id}) => {
            try{
                const {data} = await axios.put(`https://blogs-mern-v9tk.onrender.com/blogs/edit/${id}`,{
    ...
    useEffect(() => {
        const searchPostFromList = async(title) => {
            try{
                const {data} = await axios.get(`https://blogs-mern-v9tk.onrender.com/blogs/search?title=${title}`)
    ...
    useEffect(() => {
        const fetchUserBlog = async() => {
            try{
                const {data} = await axios.get(`https://blogs-mern-v9tk.onrender.com/blogs/user?fullname=${JSON.parse(sessionStorage.getItem('active')).fullname}`)
    ...
    useEffect(() => {
        const fetchUserSpecificBlog = async(name) => {
            try{
                const {data} = await axios.get(`https://blogs-mern-v9tk.onrender.com/blogs/user?fullname=${name}`)
    ...
    useEffect(() => {
        const fetchAllUsers = async() => {
            try{
                const {data} = await axios.get(`https://blogs-mern-v9tk.onrender.com/auth/authusers`);
